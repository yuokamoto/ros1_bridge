...
int main(int argc, char * argv[])
{
  ...
  //temp
  bridge_all_1to2_topics = bridge_all_2to1_topics = true;
    ...
    topics.getType() == XmlRpc::XmlRpcValue::TypeArray)
  {
    for (size_t i = 0; i < static_cast<size_t>(topics.size()); ++i) {
        topic_list.insert(static_cast<std::string>(topics[i]["topic"]));
    }
  }

  XmlRpc::XmlRpcValue services_1_to_2;
  std::set<std::string> services_1_to_2_list;
  if (
    ...
    services_1_to_2.getType() == XmlRpc::XmlRpcValue::TypeArray)
  {
    for (size_t i = 0; i < static_cast<size_t>(services_1_to_2.size()); ++i) {
        services_1_to_2_list.insert(static_cast<std::string>(services_1_to_2[i]["service"]));
    }
  }

  XmlRpc::XmlRpcValue services_2_to_1;
  std::set<std::string> services_2_to_1_list;
  if (
    ...
    services_2_to_1.getType() == XmlRpc::XmlRpcValue::TypeArray)
  {
    for (size_t i = 0; i < static_cast<size_t>(services_2_to_1.size()); ++i) {
        services_2_to_1_list.insert(static_cast<std::string>(services_2_to_1[i]["service"]));
    }
  }
  ...
  auto ros1_poll = [
    ...
    &topic_list, &services_2_to_1_list
    ](const ros::TimerEvent &) -> void
    {
      ...
      if (payload.size() >= 1) {
        for (int j = 0; j < payload[0].size(); ++j) {
          ...
          if ( topic_list.find(topic_name) == topic_list.end()) {
            continue;
          }
      ...
      if (payload.size() >= 3) {
        for (int j = 0; j < payload[2].size(); ++j) {
          if (payload[2][j][0].getType() == XmlRpc::XmlRpcValue::TypeString) {
            ...
            if ( services_2_to_1_list.find(name) == services_2_to_1_list.end()) {
              continue;
            }
  ...
  auto ros2_poll = [
    ...
    ]() -> void
    {
      ...
      for (auto topic_and_types : ros2_topics) {
        ...
        if (topic_list.find(topic_and_types.first) == topic_list.end()) {
            continue;
        }
      ...
      for (auto & pair : node_names_and_namespaces) {
        ...
        for (auto & it : services_and_types) {
          if (services_1_to_2_list.find(it.first) == services_1_to_2_list.end()) {
              continue;
          }
